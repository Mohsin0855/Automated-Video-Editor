...
text_to_video = st.sidebar.checkbox("Text to Video")
editing_services = st.sidebar.checkbox("Editing Services")

if editing_services:
    clip = st.sidebar.file_uploader(" ", type=["mp4"])

    st.sidebar.markdown("### Select the mode of operation:")
    mode_manual = st.sidebar.checkbox("Manual Editing")
    mode_automated = st.sidebar.checkbox("Automated Editing")

    if clip:
        
        if mode_manual and not mode_automated:
        ...
        elif mode_automated and not mode_manual:
            st.sidebar.write("Are you automated?")
            temp_filename = None
            with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as temp_file:
                temp_filename = temp_file.name
                temp_file.write(clip.read())
            ...
            process_options = ['trim', 'caption', 'summarize']
            selected_options = []
            for option in process_options:
                if st.sidebar.checkbox(f"Enable {option}"):
                    selected_options.append(option)
            
            if st.sidebar.button("Process Video"):
                files = {'video': open(temp_filename, 'rb')}
                data = {'process_steps': selected_options}
                response = requests.post('http://127.0.0.1:8000/process-video', files=files, data=data)
                if response.ok:
                    st.success("Video processed successfully!")
                    process_results = response.json()
                    st.write(process_results['message'])
                    if 'summary' in process_results:
                        st.write("Summary:", process_results['summary'])
                    video_path = process_results['output'].replace("\\", "/")
                    st.video(video_path)
                else:
                    st.error("Failed to process video")

        
        elif mode_manual and mode_automated:
            st.error("Please select only one mode at a time.")

elif text_to_video:
        st.sidebar.write("Text to Video")
        text = st.sidebar.text_area("Enter the text you want to convert to video")
        if st.sidebar.button("Convert to Video"):   
            data = {'prompt': text}
            response = requests.post('http://127.0.0.1:8000/generate_video', json=data)
            if response.ok:
                st.success("Video generated successfully!")
                video_path = "generated_videos/output.mp4"
                st.video(video_path)
            else:
                st.error("Failed to generate video")
else:
    st.warning("Please select a mode to proceed with video editing.")